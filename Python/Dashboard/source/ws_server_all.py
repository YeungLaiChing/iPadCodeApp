...
async def broadcast_messages():
    mobile = r.pubsub()
    mobile.subscribe(f'calc_btc_index')
    mobile2 = r.pubsub()
    mobile2.subscribe(f'crypto_index_HKBTCI-USD')
    mobile3 = r.pubsub()
    mobile3.subscribe(f'calc_eth_index')
    mobile4 = r.pubsub()
    mobile4.subscribe(f'crypto_index_HKETHI-USD')
    while True:
        ...
        if message:
            ...
            if message['data']!=1 :
                ...
                if last['BTC']<payload.get('hkt') : 
                    last['BTC']=payload.get('hkt')
        ...
        message =  mobile3.get_message()
        if message:
            current=time.time_ns()
            if message['data']!=1 :
                payload=json.loads(message['data'])
                if last['ETH']<payload.get('hkt') : 
                    last['ETH']=payload.get('hkt')
                    output = {'indexName': payload.get('id'), 'exchangeTime' : payload.get('hkt')+".000000000", 'indexValue' : payload.get('idx')}
                    await broadcast(json.dumps(output))
                #output = {'indexName': name, 'exchangeTime' : getFormattedTime(current), 'indexValue' : result}
