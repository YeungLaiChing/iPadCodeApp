...
output_path=f'./log/quote.output.{time.strftime("%Y%m%d-%H%M%S")}.csv'
...
def handleDfUpdate(df):
    ...
    for index, row in df.iterrows():
        ...
        rds.publish('quote_stream',record)
...
ret, data = quote_ctx.subscribe(['HK.00005','HK.800000','HK.800700','HK.800864','HK.800868'], [SubType.QUOTE])  # 订阅实时报价类型，OpenD 开始持续收到服务器的推送
if ret == RET_OK:
    handleUpdate(data)
    #print(data)
else:
