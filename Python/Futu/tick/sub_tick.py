...
mobile.subscribe('market_data_stream')
...
while True:
    ...
    if message:
        ...
        if message['data']!=1 :
            ...
            print(f"{payload['time']}\t : {payload['code']} = {payload['price']} tvr = {payload['turnover']}. Captured @ {getFormattedTime(ns)}. Processed @ : {getFormattedTime(current)}. Elapse time {diff_ns} ns, {diff_ms} ms")
