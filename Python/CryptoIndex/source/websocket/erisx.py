...
exchange_name=os.environ.get('EXCHANGE_NAME','erisx')
...
def process_message(message):
    types={"MarketDataIncrementalRefreshTrade_","INFO_MESSAGE"}
    try:
      content = json.loads(message)
      if (content['type'] not in types):
          print(content)
          for data in content['trades'] :
            original_timestamp = data['transactTime']
            #utc_datetime = datetime.fromtimestamp(int(original_timestamp/1000), tz=timezone.utc)
            utc_datetime = datetime.strptime(original_timestamp[0:16],'%Y%m%d-%H:%M:%S').replace(tzinfo=timezone.utc)
            ...
            trade_id=data['transactTime']
            last_price=data['price']
            last_quantity=data['size']
            side='U'
...
def get_data():
    ...
    ws=WebSocketApp("wss://publicmd-api.erisx.com/",
                    on_open=on_open,
