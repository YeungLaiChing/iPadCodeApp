...
exchange_name=os.environ.get('EXCHANGE_NAME','lmax')
...
def get_data():
    ...
    while True:
        ...
        resp = requests.get(f'https://public-data-api.london-digital.lmax.com/v1/ticker/{product_ids}')
                    
        if resp.status_code==200 :
            content=resp.json()
            if "error_message" in content: 
                print(f"Reponse  error {content['error_message']}")
                exit()
            else:
