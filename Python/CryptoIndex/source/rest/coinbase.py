...
exchange_name=os.environ.get('EXCHANGE_NAME','coinbase')
...
def get_coinbase_data():
    ...
    while True:
        if last==0:
            resp = requests.get(f'https://api.exchange.coinbase.com/products/{product_ids}/trades')
        else:
            resp = requests.get(f'https://api.exchange.coinbase.com/products/{product_ids}/trades?before={last}')
