    ...
    while True:
        ...
        if message:
            ...
            if message['data']!=1 :
                ...
                key=f"{payload['exchange']}_{payload['timestamp_org']}_{payload['from_symbol']}_{payload['to_symbol']}_{payload['trade_id']}_{float(payload['price'])}_{float(payload['volume'])}"
                #print(key)
                if rds.setnx(key,"1"):
